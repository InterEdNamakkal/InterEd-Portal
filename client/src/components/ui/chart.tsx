...
>(({ data, className, ...props }, ref) => {
  ...
  return (
    <ChartContainer ref={ref} className={cn(className)} {...props} config={chartConfig}>
      <RechartsPrimitive.BarChart data={data.labels.map((label, i) => {
        const item: Record<string, string | number> = { name: label }
        data.datasets.forEach((dataset, datasetIndex) => {
          item[`chart-${datasetIndex + 1}`] = dataset.data[i] || 0
        })
        return item
      })}>
        ...
        {data.datasets.map((dataset, index) => (
          <RechartsPrimitive.Bar 
            ...
            fill={dataset.backgroundColor || `var(--color-chart-${index + 1})`}
            radius={[4, 4, 0, 0]}
          />
        ))}
...
>(({ data, className, ...props }, ref) => {
  ...
  return (
    <ChartContainer ref={ref} className={cn(className)} {...props} config={chartConfig}>
      <RechartsPrimitive.LineChart data={data.labels.map((label, i) => {
        const item: Record<string, string | number> = { name: label }
        data.datasets.forEach((dataset, datasetIndex) => {
          item[`chart-${datasetIndex + 1}`] = dataset.data[i] || 0
        })
        ...
        {data.datasets.map((dataset, index) => (
          <RechartsPrimitive.Line
            ...
            type="monotone"
            dataKey={`chart-${index + 1}`}
            stroke={dataset.borderColor || `var(--color-chart-${index + 1})`}
            strokeWidth={2}
            dot={{ r: 4 }}
            activeDot={{ r: 6 }}
          />
        ))}
        {data.datasets.filter(d => d.fill).map((dataset, index) => (
